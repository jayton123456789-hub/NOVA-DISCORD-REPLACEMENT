...
type Props = { username: string; setUsername: (v: string) => void; onCancel?: () => void; onConnect: (c: ConnectionConfig, host?: HostStatus) => void; };

export default function ConnectScreen({ username, setUsername, onConnect, onCancel }: Props) {
  ...
  async function host() {
    ...
    if (!space.trim()) { setError('Name your Space first.'); return; }
    setBusy(true); setError('');
    try {
      const status = await invoke<HostStatus>('start_host', { spaceName: space.trim(), port: 38765 });
      onConnect({ host: '127.0.0.1', port: status.port, token: status.token, label: status.space_name, hostedSpaceId: status.space_id }, status);
  ...
  return <main className="connect-screen">
    ...
    <section className="connect-panel">{onCancel&&<button onClick={onCancel}>Back to your Space</button>}
      <label className="field-label">YOUR NAME<input value={username} maxLength={28} onChange={(e) => setUsername(e.target.value)} placeholder="Your display name"/></label>
      ...
      <small className="network-note">For friends in other homes, copy the internet invite after the host reports that internet invitations are ready. Local IP invites only work on a reachable local or VPN network.</small>
    </section>
  </main>;
}
