...
fn recent_messages(state: &HostState) -> Vec<ChatMessage> {
    let db=state.db.lock().unwrap(); let mut st=db.prepare("SELECT id,channel_id,author_id,author,body,created_at,attachment_id,attachment_name,attachment_size FROM messages ORDER BY created_at DESC LIMIT 50").unwrap();
...
async fn client_socket(socket:WebSocket,state:Arc<HostState>,name:String){
    ...
    let recipient = peer_id.clone();
    let mut send_task=tokio::spawn(async move{while let Ok(msg)=rx.recv().await{if let Ok(v)=serde_json::from_str::<Value>(&msg){if let Some(target)=v.get("target").and_then(Value::as_str){if target!=recipient{continue}}}if sender.send(WsMessage::Text(msg.into())).await.is_err(){break}}});
...
fn reply(state: &HostState, peer: &str, request: &Value, error: Option<&str>) {
    broadcast_json(state, json!({"type": if error.is_some() {"request_error"} else {"ack"}, "target": peer, "request_id": request.get("request_id"), "error": error}));
}

fn channel_exists(state: &HostState, id: &str, kind: &str) -> bool {
    state.db.lock().ok().and_then(|db| db.query_row("SELECT 1 FROM channels WHERE id=?1 AND kind=?2", params![id,kind], |_| Ok(())).ok()).is_some()
}

async fn handle_client(state:&HostState,peer_id:&str,text:&str){
    if text.len() > 65536 { return; }
    let Ok(v)=serde_json::from_str::<Value>(text) else{return};
    let t=v.get("type").and_then(Value::as_str).unwrap_or("");
    match t {
      "message"=>{
        let channel=v.get("channel_id").and_then(Value::as_str).unwrap_or("").to_string();
        let body=v.get("body").and_then(Value::as_str).unwrap_or("").chars().take(8000).collect::<String>();
        if !channel_exists(state,&channel,"text") { reply(state,peer_id,&v,Some("Select an existing text channel.")); return; }
        let attachment_id=v.get("attachment").and_then(|a| a.get("id")).and_then(Value::as_str);
        let attachment = match attachment_id {
            Some(id) => {
                let found = state.db.lock().unwrap().query_row("SELECT id,name,size FROM files WHERE id=?1 AND channel_id=?2",params![id,channel],|r|Ok(Attachment{id:r.get(0)?,name:r.get(1)?,size:r.get::<_,i64>(2)? as u64,url:String::new()})).ok();
                if found.is_none() {reply(state,peer_id,&v,Some("Attachment is not available in this channel."));return;}
                found
            }, None=>None
        };
        if body.trim().is_empty() && attachment.is_none() { reply(state,peer_id,&v,Some("Message is empty.")); return; }
        let author=state.clients.lock().unwrap().get(peer_id).map(|c|c.name.clone()).unwrap_or_default();
        let id=Uuid::new_v4().to_string(); let ts=now_ms();
        let committed = state.db.lock().unwrap().execute("INSERT INTO messages(id,channel_id,author_id,author,body,created_at,attachment_id,attachment_name,attachment_size) VALUES(?1,?2,?3,?4,?5,?6,?7,?8,?9)",params![id,channel,peer_id,author,body,ts,attachment.as_ref().map(|a| &a.id),attachment.as_ref().map(|a| &a.name),attachment.as_ref().map(|a| a.size as i64)]);
        if committed.is_err() { reply(state,peer_id,&v,Some("Host storage failed. The message was not saved. Keep your draft and retry.")); return; }
      ...
      "create_channel"=>{
        let name=safe_name(v.get("name").and_then(Value::as_str).unwrap_or(""));
        let kind=v.get("kind").and_then(Value::as_str).unwrap_or("text");
        if name.trim().is_empty()||!(kind=="text"||kind=="voice"){reply(state,peer_id,&v,Some("Enter a channel name and type."));return;}
        let id=Uuid::new_v4().to_string(); let pos=if kind=="voice"{100}else{50};
        if state.db.lock().unwrap().execute("INSERT INTO channels(id,name,kind,position) VALUES(?1,?2,?3,?4)",params![id,name,kind,pos]).is_err(){reply(state,peer_id,&v,Some("Could not save the channel."));return;}
        broadcast_json(state,json!({"type":"channels","channels":channels(state)}));reply(state,peer_id,&v,None);
      },
      "voice_join"=>{let ch=v.get("channel_id").and_then(Value::as_str).unwrap_or("").to_string();if !channel_exists(state,&ch,"voice"){return;}if let Some(c)=state.clients.lock().unwrap().get_mut(peer_id){c.voice_channel=Some(ch);}broadcast_presence(state);},
      ...
      "signal"=>{
        ...
        let allowed={let clients=state.clients.lock().unwrap();match(clients.get(peer_id),clients.get(target)){(Some(a),Some(b))=>a.voice_channel.is_some()&&a.voice_channel==b.voice_channel&&peer_id!=target,_=>false}};
        if allowed {broadcast_json(state,json!({"type":"signal","target":target,"from":peer_id,"data":v.get("data").cloned().unwrap_or(Value::Null)}));}
      },
      _=>{}
    }
}
...
async fn upload_file(State(state):State<Arc<HostState>>,Query(q):Query<AuthQuery>,mut multipart:Multipart)->impl IntoResponse{
    if q.token!=state.token{return(StatusCode::UNAUTHORIZED,"bad token").into_response()}
    let mut bytes=None;let mut filename="file.bin".to_string();let mut uploader="Unknown".to_string();let mut channel=String::new();
    ...
    if !channel_exists(&state, &channel, "text") { return (StatusCode::BAD_REQUEST, "select an existing text channel").into_response(); }
